...
function Login(source, citizenid, newData)
	...
	if QBX.Players[source] then
		...
		logger.log({
			...
			message = ('%s [%s] Dropped for attempting to login twice'):format(GetPlayerName(tostring(source)),
				tostring(source))
		})
		return false
	end
...
local function emitMoneyEvents(source, playerMoney, moneyType, amount, actionType, reason, difference)
	...
	TriggerClientEvent('hud:client:OnMoneyChange', source, moneyType, isSet and math.abs(difference) or amount,
		isSet and difference < 0 or isRemove, reason)
...
function AddMoney(identifier, moneyType, amount, reason)
	...
	if not player.Offline then
		...
		logger.log({
			...
			GetPlayerName(player.PlayerData.source), player.PlayerData.citizenid, player.PlayerData.source, amount,
				moneyType, moneyType, player.PlayerData.money[moneyType], reason),
...
function SetMoney(identifier, moneyType, amount, reason)
	...
	if not player.Offline then
		...
		logger.log({
			...
			GetPlayerName(player.PlayerData.source), player.PlayerData.citizenid, player.PlayerData.source, absDifference,
				moneyType, dirChange, moneyType, player.PlayerData.money[moneyType], reason),
		})
